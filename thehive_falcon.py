...
class TheHiveProcessor:
	...
	def thehive_alert(self,alert):
		authheader={'Content-Type': 'application/json', 'Authorization': 'Bearer '+self.conf['thehiveapi']}
		results=requests.post(self.conf['thehiveurl'],headers=authheader,data=json.dumps(alert),verify=self.conf['Verify_SSL'])
	...
	def parse_artifacts(self,event):
		artifacts=[]
		iocmap={"domain":"domain","ip":"ip","filename":"filename","command_line":"commandline","hash_sha256":"hash","registry_key":"registry"}
		...
		if "IOCType" in event and iocmap[event["IOCType"]]:
			artifact={"dataType":iocmap[event["IOCType"]],"message":"Detection IOC","data":event["IOCValue"]}
			artifacts.append(artifact)
		return artifacts
	...
	def processor(self,stream_data):
		if stream_data['metadata']['eventType'] == 'DetectionSummaryEvent':
			...
			event_time=calendar.timegm(time.gmtime((stream_data['metadata']['eventCreationTime']/1000)))
			if event_time>self.ts:
				alert_message=''
				pretty_time=datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
				title="[Falcon Host Detection - "+event["Tactic"]+"/"+event["Technique"]+"] "+event["ComputerName"]+"/"+event["UserName"]+" - "+event['DetectDescription']+" ("+pretty_time+")"
				alert={
					"title": title,
					"description":"```\n"+json.dumps(event,indent=4,sort_keys=True)+"\n```",
					"type": event['DetectName'],
					"source": event['SensorId'],
					"sourceRef": 'Falcon streaming api integration',#event['FalconHostLink'],
					"severity":2,#event["Severity"],
					"tlp":3,
	...
	while True:
		...
		try:
			if falcon_api.connect():
				...
				lh.info("Sleeping until expiry time, which is after "+str(falcon_api.sleeptime)+" seconds.")
